...
def plot_solution(nodes, u_num, u_ex, nx, ny):
    ...
    npx = nx + 1
    npy = ny + 1
    X = nodes[:, 0].reshape(npy, npx)
    Y = nodes[:, 1].reshape(npy, npx)
    U_num = u_num.reshape(npy, npx)
    U_ex = u_ex.reshape(npy, npx)
    ...
    plt.tight_layout()
    plt.savefig(f"poisson_solution_nx_{nx}.png", dpi=150)
    plt.show()
...
if __name__ == "__main__":
    ...
    for nx, ny in [(50, 50), (100, 100)]:
        solve_poisson(nx, ny, method="pardiso")
        print("-" * 60)